    ...
    def train_model(self, train_dl, epochs, display_every=200):
        ...
        for e in range(epochs):
            ...
            for data in tqdm(train_dl):
                ...
                self.optimize()
                update_losses(self, loss_meter_dict, count=data['L'].size(0)) # function updating the log objects
                i += 1
                if i % display_every == 0:
                    print(f"\nEpoch {e+1}/{epochs}")
                    print(f"Iteration {i}/{len(train_dl)}")
                    log_results(loss_meter_dict) # function to print out the losses
                    visualize(self, data, save=False) # function displaying the model's outputs